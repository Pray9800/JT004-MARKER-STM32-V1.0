...
void task_init()
{
        ...
        ws2812_set_num_spi(WS_ARRAY_SIZE, 255, 255, 255); //颜色初始化
        Sys_Delay(200);//延时
        ws2812_set_num_spi(WS_ARRAY_SIZE, 100, 100, 100); //颜色初始化
        Sys_Delay(200);//延时
        ws2812_set_num_spi(WS_ARRAY_SIZE, 50, 50, 50); //颜色初始化
...
void task()
{
    ...
    {         
        ...
        {     
            ...
            if( UR_Send_Msg.cmd == 0x0B ) // 专属亮度指令
            {
                g_brightness = UR_Send_Msg.data; 
                g_brightness_flag = 1;
            }
            else if( UR_Send_Msg.cmd == 0x0A ) // 专属颜色指令
            {        
        ...
        {
        g_timer_cnt=0;
        if(( g_rgb_sign != g_rgb_sign_last)||g_brightness_flag) //颜色指令有变化或者亮度有变化就刷新
            {
                rgb_cnt ++;//刷新次数
                if( rgb_cnt >=3)
                {
                    ...
                    g_brightness_flag = 0; //清除亮度变化标志位
                } 
                else
                        ws2812_set_num_spi(WS_ARRAY_SIZE , 
                                ((g_rgb_value>>2)&0x01)*g_brightness, 
                                ((g_rgb_value>>1)&0x01)*g_brightness, 
                                ((g_rgb_value & 0x01))*g_brightness); //一直刷新10次 100hz 频率                 
            }
        }          
                IWDG_Refresh(); //看门狗刷新       
    }      

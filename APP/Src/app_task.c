...
uint8_t uart_key[7] = {0xa5,0x5a,0x0a,0x01,0x00,0xb6,0x6b}; //测试
...
void task_init()
{
        ...
        ws2812_set_num(WS_ARRAY_SIZE, 100, 100, 100); //颜色初始化
        Sys_Delay(500);//延时
        ws2812_set_num(WS_ARRAY_SIZE, 100, 100, 100); //颜色初始化
        ...
        {     
            ...
            g_rgb_sign = UR_Send_Msg.data; //赋值
                if(g_rgb_sign == 4) //红
                    {
                        ...
                        // ws2812_set_num(18, 255, 0, 0); //颜色初始化
                        // HAL_Delay(500); //原代码方案 时隔一秒重新刷新
                        //  ws2812_set_num(18, 255, 0, 0); //颜色初始化
                        // MCU_UR_Send(& g_rgb_sign, 1);
                        //  uart_key_tt[4]=0xA4;
                        // g_rgb_sign=0xFF;
                    }
                ...
                else if(g_rgb_sign == 3)//绿 
                {
                    ...
                    //     ws2812_set_num(0, 0, 255, 0); //颜色初始化
                    //    HAL_Delay(500); //原代码方案 时隔一秒重新刷新
                    //     ws2812_set_num(0, 0, 255, 0); //颜色初始化
                    //     MCU_UR_Send(& g_rgb_sign, 1);
                    //      g_rgb_sign=0xFF;
                ...
                {     
                    ...
                    //  ws2812_set_num(18, 0, 0, 255); //颜色初始化
                        // HAL_Delay(500); //原代码方案 时隔一秒重新刷新
                        //  ws2812_set_num(18, 0, 0, 255); //颜色初始化
                        // MCU_UR_Send(& g_rgb_sign, 1);
                        //  uart_key_tt[4]=0xA2;
                ...
                
                {
                        ...
                        //     ws2812_set_num(18, 255, 255, 255); //颜色初始化
                        // HAL_Delay(500); //原代码方案 时隔一秒重新刷新
                        //      ws2812_set_num(18, 255, 255, 255); //颜色初始化
                        // MCU_UR_Send(&g_rgb_sign, 1);
                        //  uart_key_tt[4]=0xA1;
